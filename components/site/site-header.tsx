...
const nav = [
  ...
  { href: "/products#cosmetics", label: "Cosmetics" },
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-liz-gold/10 bg-background/88 backdrop-blur-xl">
      <div className="container flex h-16 items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2" aria-label="Golden Liz home">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium text-muted-foreground md:flex">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className="transition hover:text-liz-goldDeep">{item.label}</Link>
          ))}
        </nav>

        <div className="flex items-center gap-2">
          <Button asChild variant="ghost" size="icon" aria-label="Open beauty consultation">
            <Link href="/#consultation"><Sparkles className="h-5 w-5" /></Link>
          </Button>
